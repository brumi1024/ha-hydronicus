...
`;var yt=["auto","idle","heating","cooling"];var Ue=1200;var Le=E("hassConnection");var Me=E("hassApi");var Oe=E("hassConfig");var qe=E("hassInternationalization");function Ne(r){return r.replaceAll("_","-")}function ee(r){return`Retrying in ${Math.round(r/1e3)} s.`}var Y=class extends x{static properties={preview:{type:Boolean},_config:{state:true},_connection:{state:true},_unit:{state:true},_locale:{state:true},_localize:{state:true},_snapshot:{state:true},_snapshotError:{state:true},_stream:{state:true},_actionError:{state:true},_holdingShutdown:{state:true}};static styles=te;_hass;_callService;_fromContext=new Set;_plantStream;_holdTimer=null;constructor(){super();this.preview=false;this._config=void 0;this._connection=void 0;this._unit=z;this._locale=void 0;this._localize=void 0;this._snapshot=null;this._snapshotError=null;this._stream={kind:"idle"};this._actionError=null;this._holdingShutdown=false;this._plantStream=new W({onStatus:t=>this._streamStatusChanged(t),onSnapshot:t=>this._snapshotReceived(t)});new b(this,{context:Le,subscribe:true,callback:t=>{this._fromContext.add("connection");this._connection=t?.connection}});new b(this,{context:Me,subscribe:true,callback:t=>{this._fromContext.add("api");this._callService=t?.callService}});new b(this,{context:Oe,subscribe:true,callback:t=>{this._fromContext.add("config");this._unit=pt(t?.config?.unit_system)}});new b(this,{context:qe,subscribe:true,callback:t=>{this._fromContext.add("i18n");this._locale=t?.locale??(t?.language?{language:t.language}:void 0);this._localize=t?.localize}})}set hass(t){this._hass=t;if(!this._fromContext.has("connection"))this._connection=t?.connection;if(!this._fromContext.has("api"))this._callService=t?(...e)=>t.callService(...e):void 0;if(!this._fromContext.has("config"))this._unit=pt(t?.config?.unit_system);if(!this._fromContext.has("i18n")){this._locale=t?.locale??(t?.language?{language:t.language}:void 0);this._localize=t?.localize}}get hass(){return this._hass}static async getConfigForm(){return _t()}static async getStubConfig(t){return vt(t)}setConfig(t){const e=J(t);if(e.plant!==this._config?.plant){this._snapshot=null;this._snapshotError=null;this._actionError=null}this._config=e}getCardSize(){const t=this._snapshot;if(!t)return 4;let e=4;const n=Math.min(t.alerts.length,3);if(n)e+=1+n;e+=1+Math.max(1,t.zones.length)*5;if(t.delivery_paths.length)e+=1+t.delivery_paths.length*3;if(t.actuators.length)e+=1+t.actuators.length*2;e+=1;const s=Object.values(t.execution.operations).flat().length;if(s)e+=1+s;return e}getGridOptions(){return{columns:12,min_columns:6}}connectedCallback(){super.connectedCallback();this.requestUpdate()}disconnectedCallback(){this._plantStream.disconnect();this._clearHold();super.disconnectedCallback()}updated(t){super.updated(t);this._syncSelectValues();if(!this.isConnected)return;const e=this._config?.plant||void 0;this._plantStream.connect(this._connection,e);if(this._connection&&(t.has("_connection")||this.preview))void H.load(this._connection)}_syncSelectValues(){for(const t of this.renderRoot.querySelectorAll("select[data-value]")){const e=t.dataset.value??"";if(t.value!==e)t.value=e}}_streamStatusChanged(t){this._stream=t;if(["idle","unavailable","not_found","unauthorized"].includes(t.kind)){this._snapshot=null;this._clearHold()}}_snapshotReceived(t){try{this._snapshot=Ft(t);this._snapshotError=null}catch(e){this._snapshot=null;this._snapshotError=X(e,"Unsupported Hydronicus snapshot.")}}render(){const t=this._config;if(!t||!t.plant){return this._renderState("Hydronicus Plant","Select a Hydronicus Plant in the card editor.","status")}if(this._snapshotError){return this._renderState("Card update needed",this._snapshotError,"alert","Reload the browser after upgrading Hydronicus so the card and the integration match.")}const e=this._snapshot;const n=this._stream;if(!e){switch(n.kind){case"not_found":return this._renderState("Plant not found","This Hydronicus Plant was not found. Choose another Plant in the card editor.","alert");case"unauthorized":return this._renderState("No access","You do not have access to this Hydronicus Plant.","alert");case"unavailable":return this._renderState("Plant unavailable","The Hydronicus Plant is unavailable while it loads or after it was unloaded. The card reconnects automatically.","status");case"retrying":return this._renderState("Connection needs attention",n.message,"alert",ee(n.delayMs));default:return this._renderLoading(n.kind==="reconnecting")}}return a`<ha-card class=${t.density??"comfortable"} data-visual=${Vt(e)}>
      ...
      <div class="controls">
        <span class="badge ${Ne(n.mode)}"><span class="visually-hidden">Execution boundary: </span>${m(n.mode)}</span>
      ...
      <ul class="diagnostic-list" aria-label="Zone diagnostics">
        ...
        ${t.cooling.condensation_margin===null?c:a`<li class="diagnostic-chip ${t.cooling.blocked?"danger":""}" dir="auto">Margin ${It(t.cooling.condensation_margin,i,this._locale)} ${i}</li>`}
      </ul>
