  ...
  return html`<header class="header">
      <div class="plant-heading">
        ...
        <div class="header-copy">
          ...
          <h2 class="plant-title">${modeEntity ? html`<button type="button" class="link" aria-haspopup="dialog" title="Show Plant mode details" @click=${() => context.moreInfo(modeEntity)}>${plant.name}</button>` : plant.name}</h2>
      ...
      <div class="controls">
        ...
        <label class="mode-control"><span class="control-label">Mode</span><select aria-label="Requested Plant mode" data-value=${plant.requested_mode} ?disabled=${!modeEntity} @change=${modeChanged}>
...
export function renderBoundary(snapshot: PlantSnapshot): TemplateResult {
  return html`<div class="boundary" role="status">
...
export function renderAlerts(context: RenderContext, snapshot: PlantSnapshot): Rendered {
  ...
  return html`<section aria-labelledby="hydronicus-alerts"><div class="section-head"><div class="section-kicker"><h3 id="hydronicus-alerts">Alerts</h3></div><span class="meta" dir="auto">${formatNumber(alerts.length, context.locale, 0)}</span></div>${alerts.slice(0, 3).map((alert) => {
...
export function renderOperations(context: RenderContext, snapshot: PlantSnapshot): Rendered {
  ...
  return html`<section><details open><summary>Latest operation outcomes (${formatNumber(operations.length, context.locale, 0)})</summary>${operations.map((operation) => {
