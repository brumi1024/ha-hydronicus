...
function temperature(context: RenderContext, celsius: number | null, label: string, className: string): TemplateResult {
  ...
  return html`<div class=${className}><span class="metric-value">${formatTemperature(celsius, unit, context.locale)}</span><span class="metric-unit">${unit}</span><span class="metric-label">${label}</span></div>`;
}
...
export function renderRoom(context: RenderContext, zone: ZoneSnapshot, options: RoomOptions): TemplateResult {
  ...
  const heading = options.headingLevel === 2
    ? html`<h2 class="zone-title" id=${headingId}>${name}</h2>`
    : html`<h4 class="zone-title" id=${headingId}>${name}</h4>`;
  return html`<article class="zone" data-phase=${zone.phase} data-hvac-mode=${thermostat.hvac_mode ?? "unknown"} data-demand=${String(hasDemand)} data-demand-kind=${demandKind} data-blocked=${String(zone.blocked)} aria-labelledby=${headingId}>
    <div class="row"><div>${heading}<p class="meta zone-owner">${internal ? "Hydronicus thermostat" : `External thermostat · read-only${modeLabel ? ` · ${modeLabel}` : ""}`}</p></div><span class=${`phase${zone.blocked ? " state blocked" : ""}${off ? " off" : ""}`}>${phase}</span></div>
    ...
    <ul class="diagnostic-list" aria-label="Room diagnostics">
      <li class="diagnostic-chip" dir="auto">${formatNumber(zone.sensor_status.usable, locale, 0)} sensor${zone.sensor_status.usable === 1 ? "" : "s"} ready</li>
      ${zone.sensor_status.optional_excluded ? html`<li class="diagnostic-chip warning" dir="auto">${formatNumber(zone.sensor_status.optional_excluded, locale, 0)} optional excluded</li>` : nothing}
      ${zone.sensor_status.required_blocking ? html`<li class="diagnostic-chip danger" dir="auto">${formatNumber(zone.sensor_status.required_blocking, locale, 0)} required blocked</li>` : nothing}
      ${zone.cooling.dew_point === null ? nothing : html`<li class="diagnostic-chip" dir="auto">Dew point ${formatTemperature(zone.cooling.dew_point, unit, locale)} ${unit}</li>`}
      ${zone.cooling.condensation_margin === null ? nothing : html`<li class="diagnostic-chip ${zone.cooling.blocked ? "danger" : ""}" dir="auto">Margin ${formatTemperatureDelta(zone.cooling.condensation_margin, unit, locale)} ${unit}</li>`}
    </ul>
    ...
    ${internal
      ? html`${hvacModes.length
            ? html`<div class="hvac-modes" role="group" aria-label=${`${zone.name} HVAC mode`}>${hvacModes.map((mode) => html`<button type="button" class="hvac-mode" data-mode=${mode} aria-pressed=${String(mode === thermostat.hvac_mode)} ?disabled=${!controlEntity} @click=${() => chooseHvacMode(context, zone, mode)}>${hvacModeLabel(localize, mode)}</button>`)}</div>`
            : nothing}
          <div class="zone-actions">
            <button type="button" dir="ltr" ?disabled=${!canAdjust} aria-label=${`Decrease ${zone.name} target by ${step} ${unit}`} @click=${() => adjust(context, zone, -1)}>−${step}</button>
            <button type="button" dir="ltr" ?disabled=${!canAdjust} aria-label=${`Increase ${zone.name} target by ${step} ${unit}`} @click=${() => adjust(context, zone, 1)}>+${step}</button>
            ${presets.length
              ? html`<select class="preset" data-value=${thermostat.preset ?? "none"} aria-label=${`${zone.name} preset`} ?disabled=${!controlEntity} @change=${(event: Event) => choosePreset(context, zone, event)}>${["none", ...presets].map((preset) => html`<option value=${preset}>${sentenceLabel(preset)}</option>`)}</select>`
              : nothing}
          </div>`
