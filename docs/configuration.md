...
**Temperature aggregation** decides demand, but never the dew point that protects a cooling room.
Cooling safety calculates one worst-case dew point per room from its highest usable temperature and its highest usable humidity, even when different sensors report them.
...
The room climate entity reports the highest usable humidity as its current humidity.
...
The room **Cooling dew point** sensor reports the worst-case dew point, and its `dew_point_temperature` and `dew_point_humidity` attributes show the highest readings it combines.
